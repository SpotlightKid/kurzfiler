...
implements ActionListener, KeyListener
{
	...
	protected String yesCmd = Messages.getString("YesNoCancelDialog.Yes"); //$NON-NLS-1$
	protected String noCmd = Messages.getString("YesNoCancelDialog.No"); //$NON-NLS-1$
	protected String cancelCmd = Messages.getString("YesNoCancelDialog.Cancel"); //$NON-NLS-1$

	protected boolean result=false;
	protected boolean confirmed;

	public YesNoCancelDialog(Frame parent, String myName)
	{
		...
		//Buttons
		panel = new JPanel();
		JButton button = new JButton(yesCmd);
		button.setMnemonic(yesCmd.charAt(0));
		button.addActionListener(this);
		button.addKeyListener(this);
		panel.add(button);

		button = new JButton(noCmd);
		button.setMnemonic(noCmd.charAt(0));
		button.addActionListener(this);
		panel.add(button);
	...
	{
		if (event.getActionCommand().equals(yesCmd)) {
			confirmed=true;
			endDialog(true);
		}
		else if (event.getActionCommand().equals(noCmd)) {
			confirmed=false;
			endDialog(true);
		}
